...
from common.path_utils import resolve_cli_path, resolve_from_file, resolve_model_path, expand_placeholders
...
_VISION_MODEL_CACHE: dict[tuple[str, ...], tuple[Any, Any]] = {}
...
def _prompt_json_generate(config_path: Path, config: dict, messages: list[dict], tools_schema: list[dict]) -> str:
    ...
    model_path = resolve_model_path(model_setting, config_path)
    tokenizer_path = resolve_model_path(tokenizer_setting, config_path)
...
def generate_text_only(
    ...
    """
    纯文本生成（不注入工具调用格式），用于 Planner、代码生成等需要纯文本输出的场景。
    json_mode=True 时在 user 消息末尾加 JSON 提示，用于 Planner。
    返回裸字符串（模型原始输出，不做 JSON 解析）。
    """
    from copy import deepcopy
    ...
    if mode == "mock":
        # mock 模式直接返回空，让 planner fallback
        return ""
    ...
    try:
        ...
        raise RuntimeError("prompt_json mode requires torch + transformers") from exc

    model_config_dict = config.get("model", {})
    generation_config = config.get("generation", {})
    model_setting = expand_placeholders(model_config_dict.get("model_name_or_path", ""))
    tokenizer_setting = expand_placeholders(
        model_config_dict.get("tokenizer_name_or_path", model_setting)
    )
    model_path = resolve_model_path(model_setting, config_path)
    tokenizer_path = resolve_model_path(tokenizer_setting, config_path)
    ...
    local_only = bool(model_config_dict.get("local_files_only", True))
    trust_remote_code = bool(model_config_dict.get("trust_remote_code", False))
    dtype = _dtype_value(torch, str(model_config_dict.get("torch_dtype", "auto")))
    tokenizer, model = _load_model_bundle(
        AutoModelForCausalLM, AutoTokenizer,
        model_path, tokenizer_path,
        local_only, trust_remote_code, dtype,
        model_config_dict.get("device_map", "auto"),
        model_config_dict.get("max_memory"),
    )

    # 用 chat_template 但不传工具，强制文本输出
    msgs = list(messages)
...
def _load_vision_bundle(
    ...
    """
    加载支持图像输入的 Qwen3.5 模型。
    Qwen3.5 是原生多模态模型（architecture: Qwen3_5ForConditionalGeneration），
    需要用 AutoModelForImageTextToText / AutoModelForVision2Seq + AutoProcessor 而不是
    AutoModelForCausalLM + AutoTokenizer。
    与纯文本模型缓存分开（_VISION_MODEL_CACHE），因为加载类不同。
    """
    cache_key = _model_cache_key(
        model_path, tokenizer_path, local_only, trust_remote_code, dtype, device_map, max_memory,
    )
    cached = _VISION_MODEL_CACHE.get(cache_key)
    if cached is not None:
        print("vision_model_cache=hit", file=sys.stderr, flush=True)
        return cached

    print("vision_model_cache=miss", file=sys.stderr, flush=True)
    ...
    # 优先用 AutoModelForImageTextToText（新版 transformers 统一入口），
    # fallback 到 AutoModelForVision2Seq（旧版），再 fallback 到 trust_remote_code 自定义类。
    model = None
    load_errors = []
    for cls_name in ("AutoModelForImageTextToText", "AutoModelForVision2Seq"):
        try:
            ...
            load_errors.append(f"{cls_name}: {exc}")
    if model is None:
        raise RuntimeError(
            "无法使用 AutoModelForImageTextToText / AutoModelForVision2Seq 加载视觉模型，"
            "请在服务器上确认 transformers 版本支持 Qwen3_5ForConditionalGeneration 。\n"
            + "\n".join(load_errors)
        )

    _VISION_MODEL_CACHE[cache_key] = (processor, model)
...
def generate_vision_answer(
    ...
    """
    视觉问答：给定一张本地图片 + 一个问题，返回模型的文本回答。

    Qwen3.5 原生支持图像输入（chat_template 里有 image_pad 占位符处理逻辑），
    这里直接用 PIL 加载本地图片，不依赖网络 URL。

    Args:
        model_config: model.yaml 路径（复用同一份配置，model_name_or_path 指向 Qwen3.5-4B）
        image_path: 本地图片绝对路径（推荐直接传 image_reader 返回的 abs_path）
        question: 用户问题，例如"这张图里有什么？"
        mode: mock | prompt_json
    ...
    """
    if mode == "mock":
        return f"[mock] 图片 {image_path} 的描述尚未生成（mock 模式不调用真实视觉模型）"
    ...
    try:
        ...
        raise RuntimeError("vision QA requires torch + Pillow (pip install pillow)") from exc
    ...
    trust_remote_code = bool(model_config_dict.get("trust_remote_code", False))
    dtype = _dtype_value(torch, str(model_config_dict.get("torch_dtype", "auto")))
    processor, model = _load_vision_bundle(
        model_path, tokenizer_path,
        local_only, trust_remote_code, dtype,
        model_config_dict.get("device_map", "auto"),
        model_config_dict.get("max_memory"),
