...
def normalize_tool_call(tool_call: dict[str, Any], index: int = 0) -> dict:
    ...
    if not isinstance(args, dict):
        args = {}
    # 如果 args 为空，尝试从顶层字段里捕捉遗漏的参数
    # （模型有时会把 command/path 直接放到顶层）
    if not args:
        _reserved = {"id", "name", "args", "arguments", "parameters", "function"}
        extra = {k: v for k, v in tool_call.items() if k not in _reserved}
        if extra:
            args = extra
