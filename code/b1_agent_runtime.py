...
from b1_checkpoint import clear_checkpoint, save_checkpoint
...
def _validate_runtime_input(payload: dict) -> dict:
    ...
    # Validate optional prompt_patches
    patches = payload.get("prompt_patches")
    if patches is not None:
        if not isinstance(patches, list):
            raise ValueError("prompt_patches must be a JSON array")
        for i, patch in enumerate(patches):
            if not isinstance(patch, dict):
                raise ValueError(f"prompt_patches[{i}] must be an object")
            if "after_turn" not in patch or not isinstance(patch["after_turn"], int):
                raise ValueError(f"prompt_patches[{i}].after_turn must be an integer")
            if "append" not in patch and "switch_to" not in patch:
                raise ValueError(f"prompt_patches[{i}] must have 'append' or 'switch_to'")
...
def run_agent(
    ...
    # Prompt patches support
    prompt_patches = runtime.get("prompt_patches", [])
    applied_patches: set[int] = set()  # indices of patches already applied

    while True:
        # Apply any prompt_patches whose after_turn == tool_rounds (before LLM call)
        for patch_idx, patch in enumerate(prompt_patches):
            if patch_idx in applied_patches:
                continue
            if patch.get("after_turn") == tool_rounds:
                if "switch_to" in patch:
                    switch_path = resolve_from_file(patch["switch_to"], input_file)
                    messages[0]["content"] = read_text(switch_path).strip()
                    print(f"[prompt_patch] Switched system prompt from {switch_path}")
                elif "append" in patch:
                    messages[0]["content"] = messages[0]["content"] + patch["append"]
                    print(f"[prompt_patch] Appended text to system prompt (after_turn={tool_rounds})")
                applied_patches.add(patch_idx)
        ...
        # Save checkpoint after each completed tool-call turn
        save_checkpoint(
            output_dir,
            {
    ...
    # Clean up checkpoint now that the run has finished normally
